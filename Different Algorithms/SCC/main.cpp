...
/**
    ...
    Each graph and its transpose ( inverse ) has the same SCCs.

    Can be computed using floyd in O(N^3) time =>> [
        =>check for each 2 nodes if is[i][j] && is[j][i] then they belong to the same SCC
    ]

    Run DFS from each node and mark all nodes which are visitable from this node
        =>check for each 2 nodes if is[i][j] && is[j][i] then they belong to the same SCC

    Tarjan using 1 DFS call =>> can be used to detect bridges and articulation points

    [
        first thing
        => assign number of visiting time for each node

        whenever visiting any node there is 3 cases [
            not visited then i am parent to it [ add to the stack ]
            visited and i am ancestor to it[ in stack ] => cycle
            visited and i am not ancestor to it [ don't give attention to it ]=> not cycle
    ...
    Component Graph [
        ...
        makes every SCC as one node in a component
            can be used in how to transfer the whole graph to SCC ( first compute component graph and then see how to make these (computed nodes) reachable from each other )
            can be used in transitive closure
...
void tarjan(int node) {
	...
	rep(i, adjList[node]) {
        ...
        /** see here
            ...
            both works but the first is better cuz it works for SCC, bridges, articulation points
